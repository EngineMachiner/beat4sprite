...
local tParticleInfo = {}	
	...
	Name="Particle"..i;
		...
		:setstate(numStates):SetAllStateDelays(1/numDelay*2):zoom(0):sleep(0.5)
		:playcommand("Phase2")

		end;
		
		Phase2Command=function(self)
			self:xy(-self:GetWidth()/1.75,SCREEN_HEIGHT/((self:GetHeight()/2)/(self:GetHeight()/(16/i)))):zoom(1.5*ScaleVar)
				:linear(1)
				:xy(SCREEN_CENTER_X,SCREEN_CENTER_Y):zoom(0):sleep(0.5)
				:playcommand("Phase3")
		end;
				
		Phase3Command=function(self)
			self:xy(SCREEN_WIDTH/((self:GetWidth()/1.75)/(self:GetWidth()/(16/i))),SCREEN_HEIGHT+self:GetHeight()/2):zoom(1.5*ScaleVar)
				:linear(1)
				:xy(SCREEN_CENTER_X,SCREEN_CENTER_Y):zoom(0):sleep(0.5)
				:playcommand("Phase4")
		end;		
		...
		Phase4Command=function(self)
			self:xy(SCREEN_WIDTH+self:GetWidth()/1.75,(-SCREEN_HEIGHT/((self:GetHeight()/2)/(self:GetHeight()/(16/i)))+SCREEN_HEIGHT)):zoom(1.5*ScaleVar)
				:linear(1)
				:xy(SCREEN_CENTER_X,SCREEN_CENTER_Y):zoom(0):sleep(0.5)
		end;		
