...
local t = Def.ActorFrame {
	...
	a .. { OnCommand=function(self)
					 self:x(scale(3,0,4,SCREEN_LEFT,SCREEN_RIGHT)):zoomx(-1):y(scale(1,0,4,SCREEN_TOP,SCREEN_BOTTOM))
					 if self.SetDecodeMovie then self:SetDecodeMovie(false) end
			 end };
	a .. { OnCommand=function(self)
					 self:x(scale(1,0,4,SCREEN_LEFT,SCREEN_RIGHT)):zoomy(-1):y(scale(3,0,4,SCREEN_TOP,SCREEN_BOTTOM))
					 if self.SetDecodeMovie then self:SetDecodeMovie(false) end
			 end };
	a .. { OnCommand=function(self)
					 self:x(scale(3,0,4,SCREEN_LEFT,SCREEN_RIGHT)):zoomx(-1):zoomy(-1):y(scale(3,0,4,SCREEN_TOP,SCREEN_BOTTOM))
					 if self.SetDecodeMovie then self:SetDecodeMovie(false) end
			 end };
};
